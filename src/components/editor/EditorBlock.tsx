...
export function EditorBlock({
  ...
  // Check for markdown shortcuts
  const checkMarkdownShortcuts = (text: string): { type: BlockType; content: string } | null => {
    ...
    if (text.startsWith("- ") || text.startsWith("* ")) return { type: "bulletList", content: text.slice(2) };
  ...
  const renderBlockContent = () => {
    ...
    const baseClasses = "outline-none w-full min-h-[1.5em] empty:before:content-[attr(data-placeholder)] empty:before:text-muted-foreground";
    ...
    const typeClasses: Record<BlockType, string> = {
      ...
      bulletList: "text-base pl-6 before:content-['•'] before:absolute before:left-0 before:text-muted-foreground relative",
    ...
    return (
      <div
        ...
        onMouseUp={handleMouseUp}
        onClick={onSelect}
        data-placeholder={block.type === "paragraph" ? "Írj valamit, vagy '/' a menühöz..." : ""}
