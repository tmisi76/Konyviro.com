  ...
  return useQuery({
    ...
    queryFn: async (): Promise<Invoice[]> => {
      ...
      const { data, error } = await supabase
        .from('profiles')
        .select('id, display_name, user_id, subscription_tier, subscription_status, created_at, updated_at')
      ...
      return (data || []).map(profile => ({
        id: profile.id,
        customer_email: profile.display_name || profile.user_id.substring(0, 8),
