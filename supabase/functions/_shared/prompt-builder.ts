...
export function buildFictionStylePrompt(fictionStyle: Record<string, unknown> | null, subcategory: string | null): string {
  ...
  // Add new-character naming guide based on chosen nationality + setting
  // This ensures NEW characters introduced mid-book follow the same culture
  // as the initial cast generated by generate-story.
  const nationality = (fictionStyle.characterNationality as string | undefined) ?? null;
  const setting = (fictionStyle.setting as string | undefined) ?? null;
  if (nationality || setting) {
    parts.push(buildNewCharacterNamingGuide(nationality, setting, []));
  }
